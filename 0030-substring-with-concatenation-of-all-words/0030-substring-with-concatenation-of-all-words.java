    ...
    public List<Integer> findSubstring(String s, String[] words) {
        ...
        for (int i = 0; i < w; i++) {
            ...
            for (int ed = i; ed + w <= s.length(); ed += w) {
                // 원본 string을 words 요소 크기만큼 잘라 비교
                String str = s.substring(ed, ed + w);
                if (need.containsKey(str)) {
                    cnt++; // key가 있으면 현재 카운트 증가
                    // need처럼 키의 값 or 기본값 0에 카운트 1 증가
                    curr.put(str, (curr.getOrDefault(str, 0) + 1));
                    // 총 필요 개수를 넘어가면 while로 
                    // 그 key의 현재 카운트를 1 줄이고 맨 앞 중복 단어를 제거
                    ...
                    // 만족하는 총 단어 수가 필요한 총 단어 수와 일치: 
                    // 리스트에 시작 인덱스 추가 (equals 함수는 비효율적)
                    if (cnt == words.length) {
                        indices.add(st);
                    }
                } else {
                    cnt = 0; // key가 없으면 카운트 초기화
                    st = ed + w; // 시작점 이동
